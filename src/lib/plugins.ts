...
function guessGitHubUrl(pluginName: string): string {
  // Common publishers
  ...
  if (publisher) {
    return `https://github.com/${publisher}/${pluginName}`;
  }

  // Fallback: search GitHub
  return `https://github.com/search?q=${encodeURIComponent(pluginName)}&type=repositories`;
}
