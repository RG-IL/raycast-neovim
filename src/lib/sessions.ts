...
function getShadaEntries(): SessionEntry[] {
  if (!fs.existsSync(SHADA_PATH)) return [];

  try {
    const stat = fs.statSync(SHADA_PATH);
    return [
      {
        id: "shada:last",
        name: "Last Neovim Session",
        path: os.homedir(),
        lastOpened: stat.mtime,
        source: "shada" as const,
      },
    ];
  } catch {
    return [];
...
export function useSessions() {
  const { data, isLoading, error, revalidate } = usePromise(async () => {
    ...
    const shada = getShadaEntries();
    return mergeSessions(persistence, recent, shada);
  });

  async function removeEntry(entry: SessionEntry) {
    if (entry.source === "recent" && fs.existsSync(RECENT_DIRS_PATH)) {
      try {
        const data = fs.readFileSync(RECENT_DIRS_PATH, "utf-8");
        const entries: RecentDirEntry[] = JSON.parse(data);
